...
const OPEN_SOURCE_CANDIDATE_MODELS = [
  ...
  // 1. Primary verified active high-speed models on Groq & open endpoints
  'openai/gpt-oss-20b',
  'openai/gpt-oss-120b',
  'qwen/qwen3.6-27b',
  'qwen/qwen3.8-27b',
  ...
  for (const model of modelsToTry) {
    try {
      ...
      const isTransient =
        ...
        err?.code === 503 ||
        err?.code === 429 ||
        (err?.message && (err.message.includes('high demand') || err.message.includes('quota') || err.message.includes('RATE_LIMIT') || err.message.includes('resource exhausted')));
...
function extractJSON(text: string): any {
  if (!text) return {};
  // Strip reasoning/think blocks if present (e.g. from DeepSeek or Qwen)
  ...
  try {
    ...
    if (firstBracket !== -1 && lastBracket > firstBracket) {
      const arraySlice = cleanedText.slice(firstBracket, lastBracket + 1);
      try {
...
You are a children's book author specializing in short, heartwarming stories for children aged 2 to 7 (Toddlers, Preschoolers, & Early Readers). Your core mandate is:
"Write a short, heartwarming story for children written in Simple Present and Past Tenses using lighthearted, vivid language. Write in short, clear sentences using correct grammar and simple, descriptive language."

Follow these strict writing rules for Ages 2-7:

OVERARCHING SYNOPSIS & INCITING INCIDENT MANDATE (AGES 2-7):
- KEEP OVERARCHING SYNOPSIS AND INCITING INCIDENT NOT TOO COMPLEX:
  * Avoid complex mechanics, high-stakes danger, multi-step engineering, abstract science, or convoluted plots.
  * Inciting incidents must be simple, charming, and relatable for toddlers and early readers (e.g., finding a lost toy, a friendly puppy looking for its ball, baking cookies for woodland friends, a colorful kite caught in an apple tree, or discovering a cozy hidden door in the garden).
  * Use simple words and playful, cheerful phrases to keep kids interested, curious, and actively engaged throughout.
  * The overarching synopsis must be 1 to 2 short, crisp sentences written in simple present or simple past tense.

Target Grammar & Vocabulary (Ages 2-7):
- WRITE IN SIMPLE PRESENT AND PAST TENSES: Use active Simple Present (e.g., "Milo sees a glowing key. He picks it up with a smile.") or Simple Past tense (e.g., "Milo found a golden bird. The bird sang a sweet song."). Avoid complex tenses, passive voice, or nested clause structures.
- SHORT & CLEAR SENTENCES: Write short, punchy sentences (5–10 words average). Ensure every sentence uses correct grammar, clean structure, and crystal-clear meaning.
- SIMPLE, DESCRIPTIVE LANGUAGE: Use vivid, lighthearted, and descriptive language that a 2-to-7-year-old child can easily understand when reading or listening. Limit adjectives to 1 per sentence maximum, relying on colorful action verbs.
- HEARTWARMING & LIGHTHEARTED TONE: Maintain a heartwarming, joyful, and gentle atmosphere filled with kindness, wonder, positive choices, and cozy reassurance.
...
- Maintain a warm, inviting, lighthearted, and playful tone throughout.

Narrative & Pacing:
- Focus on short, heartwarming, relatable plot lines with clear cause-and-effect sequences.
...
Formatting for Early Readers (Ages 2-7):
- Use short paragraphs (2 to 4 sentences maximum).
- Break up heavy blocks of text to make the page look approachable and inviting for young children.
`;
...
- Ages 2–7 (Toddler, Preschool, & Early Reader):
  * OVERARCHING SYNOPSIS & INCITING INCIDENT: Keep it simple, gentle, and relatable (e.g. finding a lost toy, helping a friendly animal, sharing a sweet snack, exploring a sunlit garden). Avoid complex technical challenges, high-stress dilemmas, or multi-step engineering.
  * LANGUAGE & PROSE: Write a short, heartwarming story written in Simple Present and Past Tenses using lighthearted, vivid language. Use short, clear sentences (5–10 words average) using correct grammar and simple words and phrases to keep kids interested, curious, and engaged. Zero purple prose, active verbs, cozy heartwarming tone, and clear cause-and-effect.
...
function generateProceduralChapter(options: {
  ...
  // Kid-Friendly Stage Beats for Ages 2-7 (Simple vocabulary, heartwarming themes, zero complex mechanics)
  const kidsStageBeats = [
    {
      title: 'A Sunny Morning Surprise',
      buildContent: () =>
        `The morning sun was warm and bright. ${lead.name} put on their cozy little shoes and stepped outside with a big smile.\n\n` +
        `"Look!" said ${lead.name}. Right by the soft green grass, a cheerful shiny pebble sparkled in the flowers. ${companion ? `${companion.name} hopped over with a giggle to see it too.` : 'It gleamed like a drop of golden sunshine.'}\n\n` +
        `${lead.name} gently picked it up. A fun, happy adventure had just begun!`,
      getScenePrompt: (l: any, c: any, lAnchors: string, cAnchors: string) =>
        `${l.name} (${lAnchors}) smiling happily in a sunny green garden, gently picking up a warm glowing pebble from yellow buttercups ${c ? `with ${c.name} (${cAnchors}) watching with wide joyful eyes` : ''}.`,
    },
    {
      title: 'Skipping Down the Garden Path',
      buildContent: () =>
        `Hop, skip, and jump! ${lead.name} trotted down the smooth pebble path.\n\n` +
        `"Listen," whispered ${companion ? companion.name : lead.name}. A tiny bluebird chirped a sweet, cheerful song from an apple branch. ${cleanAction ? `Together, they chose to ${cleanAction.toLowerCase()}.` : 'The little bird fluttered its wings in a friendly hello.'}\n\n` +
        `${lead.name} waved back with a laugh and skipped merrily ahead.`,
      getScenePrompt: (l: any, c: any, lAnchors: string, cAnchors: string) =>
        `${l.name} (${lAnchors}) happily skipping along a smooth garden path under sunny flowering trees, waving at a friendly singing bluebird ${c ? `with ${c.name} (${cAnchors})` : ''}.`,
    },
    {
      title: 'A Friendly Forest Pal',
      buildContent: () =>
        `A soft rustle came from the clover patch. Out popped a friendly bunny with long floppy ears!\n\n` +
        `"Hello little friend," said ${lead.name} gently. The bunny wiggled its cute pink nose and pointed toward a hidden wooden garden gate.\n\n` +
        `${lead.name} ${companion ? `and ${companion.name}` : ''} clapped their hands with pure delight.`,
      getScenePrompt: (l: any, c: any, lAnchors: string, cAnchors: string) =>
        `${l.name} (${lAnchors}) kneeling gently in clover to say hello to an adorable fluffy bunny ${c ? `beside ${c.name} (${cAnchors})` : ''} next to a cute wooden gate with bright wildflowers.`,
    },
    {
      title: 'Working Together with a Smile',
      buildContent: () =>
        `The little garden gate had a tiny wooden latch that was just a tiny bit stuck.\n\n` +
        `"We can do this together!" cheered ${lead.name}. With steady, careful hands, ${lead.name} lifted the latch ${companion ? `while ${companion.name} gave a gentle push` : 'with care'}.\n\n` +
        `*Click!* The gate swung open smoothly, revealing a secret meadow filled with sweet wild strawberries!`,
      getScenePrompt: (l: any, c: any, lAnchors: string, cAnchors: string) =>
        `${l.name} (${lAnchors}) and ${c ? `${c.name} (${cAnchors})` : 'their companion'} opening a cute rustic wooden gate together with joyful smiles, discovering giant red strawberries and dancing butterflies.`,
    },
    {
      title: 'Sharing a Sweet Treat',
      buildContent: () =>
        `The air smelled like sweet berries and warm honey. ${lead.name} picked the plumpest red berries to share.\n\n` +
        `"One for you, one for me, and one for our bunny friend!" said ${lead.name}. ${companion ? `${companion.name} giggled and took a juicy bite.` : 'Everyone tasted the delicious sweetness.'}\n\n` +
        `Sharing made everyone's smiles even bigger and brighter.`,
      getScenePrompt: (l: any, c: any, lAnchors: string, cAnchors: string) =>
        `${l.name} (${lAnchors}) sitting on a soft grassy hill sharing big ripe red strawberries with ${c ? `${c.name} (${cAnchors}) and ` : ''}a cute woodland bunny under gentle sunshine.`,
    },
    {
      title: 'The Cozy Treehouse Playhouse',
      buildContent: () =>
        `Under a tall green willow tree stood a cozy playhouse made of polished wood and soft moss.\n\n` +
        `"Look at the door!" shouted ${lead.name}. Right in the center was a little round spot that fit the shiny pebble perfectly!\n\n` +
        `${lead.name} tapped the pebble in place. A soft chime rang out, and warm fairy lights twinkled all around the roof!`,
      getScenePrompt: (l: any, c: any, lAnchors: string, cAnchors: string) =>
        `${l.name} (${lAnchors}) placing the golden pebble into a whimsical mossy wooden treehouse door as warm fairy lights glow with joy ${c ? `beside ${c.name} (${cAnchors})` : ''}.`,
    },
    {
      title: 'A Warm Sunset Walk Home',
      buildContent: () =>
        `The sky turned soft peach and warm pink as the afternoon came to an end.\n\n` +
        `"What a happy day," whispered ${lead.name}. ${companion ? `${companion.name} held their hand with a warm smile.` : `${lead.name} gave a cozy little yawn.`}\n\n` +
        `They strolled back along the quiet garden path, listening to the crickets beginning their evening lullaby.`,
      getScenePrompt: (l: any, c: any, lAnchors: string, cAnchors: string) =>
        `${l.name} (${lAnchors}) walking hand-in-hand along a peaceful garden path under a stunning warm peach and pastel pink sunset sky ${c ? `with ${c.name} (${cAnchors})` : ''}.`,
    },
    {
      title: 'Safe and Cozy in Bed',
      buildContent: () =>
        `Back home, tiny silver stars twinkled through the bedroom window. ${lead.name} cuddled under a warm, fluffy quilt.\n\n` +
        `"Goodnight little bird, goodnight friendly bunny," whispered ${lead.name} softly.\n\n` +
        `With a peaceful, happy heart, ${lead.name} closed their eyes for sweet and joyful dreams.`,
      getScenePrompt: (l: any, c: any, lAnchors: string, cAnchors: string) =>
        `${l.name} (${lAnchors}) tucked comfortably into a cozy bed with a warm quilt, looking out a bedroom window at twinkling gentle night stars with a peaceful, happy smile.`,
    },
  ];

  // Select beat index deterministically per chapterNumber
  const isKidsAudience = targetAudience === '2-4' || targetAudience === '5-7' || targetAudience === 'kids_preschool' || targetAudience === 'kids_early';
  const effectiveBeats = isKidsAudience ? kidsStageBeats : stageBeats;
  const beatIndex = Math.min(chapterNumber - 1, effectiveBeats.length - 1);
  const currentBeat = effectiveBeats[beatIndex] || effectiveBeats[effectiveBeats.length - 1];
...
Generate a JSON object where 'characters' is an array of character objects matching the requested schema.`;
    ...
    try {
      ...
      // Auto-render 4D Pixar 3D animated character portrait for each extracted character
      for (const c of charList) {
        try {
          const appearanceStr = Array.isArray(c.appearanceTags) ? c.appearanceTags.join(', ') : '';
          const pixarPrompt = `4D Pixar 3D animated film character portrait rendering, masterpiece 3D animation still, Physically-Based Rendering (PBR), micro-texture detail, subsurface scattering on skin, expressive stylized character design, warm cinematic volumetric studio lighting, rich vibrant color palette, octane render style. Character portrait of ${c.name || 'Hero'}, ${c.gender || 'character'} ${c.speciesOrArchetype || 'persona'}, featuring ${appearanceStr}. ${c.artisticStylePrompt || ''}. Single character hero portrait, centered, 3D animated film style, soft background, zero text, zero watermark.`;

          const portraitResult = await generateImageMultiTier(pixarPrompt, {
            width: 800,
            height: 800,
            aspectRatio: '1:1',
            seed: Math.floor(Math.random() * 888888),
          });
          if (portraitResult?.imageUrl) {
            c.pixarPortraitUrl = portraitResult.imageUrl;
            c.photoUrl = portraitResult.imageUrl;
          }
        } catch (imgErr) {
          console.warn('Auto 4D Pixar rendering note:', imgErr);
        }
      }

      return res.json({ success: true, characters: charList, character: charList[0], provider: 'gemini+4d-pixar-render' });
    } catch (geminiErr: any) {
...
// Endpoint: Generate / Render 4D Pixar Character Portrait
app.post('/api/character/generate-portrait', async (req, res) => {
  try {
    const {
      name = 'Hero',
      gender = 'girl',
      speciesOrArchetype = 'Explorer',
      appearanceTags = [],
      artisticStylePrompt = '',
      keyColors = ['#f59e0b', '#06b6d4', '#10b981'],
    } = req.body;

    const appearanceStr = Array.isArray(appearanceTags) ? appearanceTags.join(', ') : '';
    const pixarPrompt = `4D Pixar 3D animated film character portrait rendering, masterpiece 3D animation still, Physically-Based Rendering (PBR), micro-texture detail, subsurface scattering on skin, expressive stylized character design, warm cinematic volumetric studio lighting, rich vibrant color palette, octane render style. Character portrait of ${name}, ${gender} ${speciesOrArchetype}, featuring ${appearanceStr}. ${artisticStylePrompt}. Color palette accents: ${keyColors.join(', ')}. Single character hero portrait, centered, high detail 3D animated film rendering, soft vignette background, zero text, zero watermark, zero captions.`;

    const result = await generateImageMultiTier(pixarPrompt, {
      width: 800,
      height: 800,
      aspectRatio: '1:1',
    ...
    console.error('Error in /api/character/generate-portrait:', error);
    return res.status(500).json({ error: error.message || 'Failed to generate 4D Pixar portrait' });
  }
});
...
- ${leadName}: Speech Pattern: "${cast[0]?.speechPattern || 'Thoughtful and observant'}". Carries: "${cast[0]?.signatureItem || 'a treasured token'}".`;
    ...
    if (isChildrenStory) {
      const ageDetails =
        targetAudience === 'kids_preschool' || targetAudience === '2-4'
          ? 'Ages 2-4 (Toddlers & Preschool): Write a short, heartwarming story for children written in Simple Present and Past Tenses using lighthearted, vivid language. Write in short, clear sentences using correct grammar and simple, descriptive language.'
          : targetAudience === 'kids_early' || targetAudience === '5-7'
          ? 'Ages 5-7 (Early Readers): Write a short, heartwarming story for children written in Simple Present and Past Tenses using lighthearted, vivid language. Write in short, clear sentences using correct grammar and simple, descriptive language.'
          : targetAudience === 'kids_middle' || targetAudience === '8-10'
...
================================================================================
CHILDREN'S STORYTELLING & EARLY READER MANDATES (AGES 2-7 FOCUS):
================================================================================
...
- MANDATORY GRAMMAR & STYLE RULE: Write a short, heartwarming story for children written in Simple Present and Past Tenses using lighthearted, vivid language. Write in short, clear sentences using correct grammar and simple, descriptive language.
...
app.post('/api/story/generate-premises', async (req, res) => {
  try {
    ...
    const isKidsAge = ageRange === '2-4' || ageRange === '5-7' || ageRange === 'kids_preschool' || ageRange === 'kids_early' || ageRange === 'early_reader';

    const prompt = isKidsAge
      ? `You are a master children's book author crafting charming story ideas for young children aged 2 to 7 (Toddlers & Early Readers).
Follow the KIDS STORY PREMISE MANDATE (AGES 2-7):
1. Simple Inciting Incident: Keep the inciting incident gentle, friendly, and very easy to understand (e.g., a lost red boot, a puppy looking for a sunny spot, baking berry muffins for woodland friends, a runaway yellow balloon, or finding a colorful key to a cozy treehouse).
2. Not Too Complex: Strictly avoid complex mechanics, high-stakes danger, multi-step engineering, abstract science, or convoluted plots.
3. Simple Words & Engaging Phrases: Use simple, lighthearted words and joyful phrases that keep young children interested, curious, and excited to listen.
4. Short Synopsis: Write 1 to 2 short, crisp sentences in Simple Present or Simple Past tense (5–12 words per sentence average).

Parameters:
Target Age Range: ${ageRange} (Ages 2-7 Early Reader / Toddler)
Genre / Atmosphere: ${genreMashup}
Cast Members: ${cast.map((c: any) => `${c.name} (${c.titleOrRole || 'Hero'})`).join(', ') || 'A friendly young explorer'}

Return a JSON object with a 'premises' array containing 3 objects with 'title' and 'synopsis'.`
      : `You are a master story strategist for children's books.
...
Return a JSON object with a 'premises' array containing 3 objects with 'title' and 'synopsis'.`;
    ...
    try {
      ...
      console.warn('Gemini premise generation failed, attempting Groq fallback:', e?.message);
      if (process.env.GROQ_API_KEY) {
        try {
          const groqText = await callGroqFallback([
            {
              role: 'system',
              content: isKidsAge
                ? "You are a children's storybook author for ages 2-7. Keep overarching synopsis and inciting incident simple, clear, and heartwarming. Use simple words and phrases to keep kids interested and engaged. Reply strictly with JSON containing a 'premises' array of 3 objects with 'title' and 'synopsis'."
                : "You are a master story strategist for children's books. Reply strictly with JSON containing a 'premises' array of 3 objects with 'title' and 'synopsis'.",
            },
            { role: 'user', content: prompt },
          ], true);

          const parsedGroq = extractJSON(groqText);
          if (Array.isArray(parsedGroq.premises) && parsedGroq.premises.length > 0) {
            return res.json({ success: true, premises: parsedGroq.premises, provider: 'groq' });
          }
        } catch (groqErr) {
          console.warn('Groq premise generation fallback note:', groqErr);
        }
      }
    }

    // Fallback premises adhering strictly to age appropriateness
    const leadHeroName = cast[0]?.name || 'Milo';
    const fallbackPremises = isKidsAge
      ? [
          {
            title: `The Great Berry Basket Hunt`,
            synopsis: `When ${leadHeroName} picks sweet blue berries, a friendly squirrel gently borrows the basket. ${leadHeroName} follows tiny paw prints through the sunny garden to share a yummy snack.`,
          },
          {
            title: `The Mystery of the Flying Yellow Kite`,
            synopsis: `A warm breeze lifts a bright yellow kite into a cozy apple tree. ${leadHeroName} and a happy puppy work together to safely untangle the ribbon and fly it high again.`,
          },
          {
            title: `The Shiny River Pebble`,
            synopsis: `${leadHeroName} finds a smooth river pebble that glows like the morning sun. Following its warm shine along the grassy brook, they discover a cozy hidden duckling nest.`,
          },
        ]
      : [
          {
            title: `The Water-Wheel Riddle of Greenhaven`,
            synopsis: `When a mysterious clogged valve halts the community hydroponic farm, ${leadHeroName} must navigate high water pressures and construct a bamboo siphon before nightfall.`,
          },
          {
            title: `The Lost Blueprint of the Sky-Sail`,
            synopsis: `An unexpected storm tears the primary canvas on the local airship cargo carrier. ${leadHeroName} uses recycled sailcloth and aerodynamic folding to restore flight stability.`,
          },
          {
            title: `The Mystery of the Silent Clocktower`,
            synopsis: `A tiny brass gear slips out of the central clock tower during the town festival. ${leadHeroName} tracks environmental clues through rain-slicked cobbled alleys to rebuild the escapement mechanism.`,
          },
        ];
...
Synopsis & History: ${synopsis} ${chaptersSummary}`;
    ...
    try {
      ...
      console.warn('Gemini editorial blurb failed, attempting Groq fallback:', e?.message);
      if (process.env.GROQ_API_KEY) {
        try {
          const groqText = await callGroqFallback([
            { role: 'system', content: `${systemInstruction}\nYou must reply strictly with a JSON object containing: blurb, castTeaser, thematicTags, recommendedReaderAge.` },
            { role: 'user', content: userPrompt },
          ], true);

          const parsedGroq = extractJSON(groqText);
          if (parsedGroq && parsedGroq.blurb) {
            return res.json({ success: true, ...parsedGroq, provider: 'groq' });
          }
        } catch (groqErr) {
          console.warn('Groq blurb generation fallback note:', groqErr);
        }
      }
...
app.post('/api/video/animate', async (req, res) => {
  try {
    ...
    try {
      const res = await callGeminiContentResilient({
